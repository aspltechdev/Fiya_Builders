...
const ProjectsGridSection = ({
  ...
  useEffect(() => {
    ...
    const animateElements = document.querySelectorAll(".uniywwu-grid__animate");
    
    // If IntersectionObserver is not available, show everything
    if (!("IntersectionObserver" in window)) {
      animateElements.forEach((el) => {
        el.classList.add("uniywwu-grid__animate--visible");
      });
      return;
    }
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.classList.add("uniywwu-grid__animate--visible");
  ...
  }, [projects, loading]); // Re-run when projects or loading changes
  ...
  const getDelayClass = (index) => {
    ...
    return `uniywwu-grid__animate--${delays[delayIndex]}`;
  };

  return (
    <section className="uniywwu-grid" id="projects" ref={gridRef}>
      <div className="uniywwu-grid__container">
        {/* Toolbar */}
        <div className="uniywwu-grid__toolbar">
          <div className="uniywwu-grid__label uniywwu-grid__animate">
            <span className="uniywwu-grid__label-number">02</span>
            <span className="uniywwu-grid__label-line" />
            <span className="uniywwu-grid__label-text">SELECTED WORK</span>
          </div>
          
          <div className="uniywwu-grid__filters uniywwu-grid__animate uniywwu-grid__animate--delay-1">
            {categories.map((category) => (
              <button
                key={category}
                type="button"
                className={`uniywwu-grid__filter-btn ${
                  activeFilter === category ? "uniywwu-grid__filter-btn--active" : ""
                }`}
                onClick={() => setActiveFilter(category)}
              >
                {category}
        ...
        {/* Loading State */}
        {loading ? (
          <div className="uniywwu-grid__loading">
            <div className="uniywwu-grid__spinner" />
            <span>Loading projects...</span>
          </div>
        ) : projects.length === 0 ? (
          <div className="uniywwu-grid__empty">
            <span className="uniywwu-grid__empty-label">NO PROJECTS FOUND</span>
            <p className="uniywwu-grid__empty-text">
              Projects will appear here once they are added from the admin panel.
            </p>
          </div>
        ) : (
          /* Projects Grid */
          <div className="uniywwu-grid__grid">
            {projects.map((project, index) => {
              const image = getProjectImage(project);
              const isWide = index % 3 === 0;
              ...
              return (
                <article 
                  className={`uniywwu-grid__card ${
                    isWide ? "uniywwu-grid__card--wide" : ""
                  } uniywwu-grid__animate ${delayClass}`}
                  key={project.id || project._id || index}
                >
                  <a
                    href={`/projects/${project.slug || project._id || project.id}`}
                    className="uniywwu-grid__card-link"
                  >
                    <div className="uniywwu-grid__card-image">
                      {image ? (
                        <img
                          src={image}
                          alt={project.title || "Project"}
                          className="uniywwu-grid__card-img"
                          onError={(e) => {
                            console.error(`Image failed to load: ${image}`);
                            e.target.src = "https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=800&q=80";
                          }}
                        ...
                        <div className="uniywwu-grid__card-placeholder">
                          <span>FIYA</span>
                        </div>
                      )}
                      
                      <div className="uniywwu-grid__card-overlay">
                        <span className="uniywwu-grid__card-year">
                          {project.year || "2024"}
                        </span>
                        <span className="uniywwu-grid__card-view">
                          VIEW PROJECT
                          <ArrowUpRight size={18} className="uniywwu-grid__card-view-icon" />
                        </span>
                      </div>
                    </div>
                    
                    <div className="uniywwu-grid__card-info">
                      <div className="uniywwu-grid__card-info-left">
                        <span className="uniywwu-grid__card-category">
                          {project.category || project.type || "PROJECT"}
                        </span>
                        <h3 className="uniywwu-grid__card-title">
                          {project.title || project.name}
                        </h3>
                      </div>
                      
                      <div className="uniywwu-grid__card-location">
                        <MapPin size={14} className="uniywwu-grid__card-location-icon" />
                        <span>{project.location || "Pondicherry"}</span>
                      </div>
                    </div>
                  </a>
